        ...
        <div class="max-w-3xl mx-auto p-6 bg-gray-50 rounded-lg shadow-lg">
            ...
            <form @submit.prevent="submitForm" class="flex flex-col gap-4">
                ...
                <button
                    ...
                    type="submit"
                    class="w-24 mx-auto p-3 font-semibold text-white bg-blue-600 rounded-md hover:bg-blue-700 transition-colors mt-4"
                >
                    Create Thread
                </button>
            </form>

            <div v-if="form.invalid('title')" class="text-red-500 text-sm mt-2">
                {{ form.errors.title }}
            </div>
            <div
            ...
            <div
                v-if="form.invalid('postContent')"
                class="text-red-500 text-sm mt-2"
            >
                {{ form.errors.postContent }}
            </div>
        </div>
    </ForumLayout>
</template>
...
const form = useForm(
    'post',
    route('threads.store', { category: props.category.id }),
    {
        title: '',
